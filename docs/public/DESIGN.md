...
> 版本：v0.9.4（已发布稳定版）/ v0.9.5（代码库候选）｜设计状态：✅ 已落地 / ⚠️ 已写待补完 / 🔲 设计草案（待实现）
> 审阅视角：高级工程师 / 高级架构师
...
> **v0.9.5（2026-09-24，代码库候选/未发布）**：优化 Trae / Cursor 智能体协同交互，消除手动调用 MCP 的误解；补齐前端一次性 SDK 接入指引与安全警告；虚拟帧扫描守卫核心修复已合入 main 分支（commit `390849f` / `2dc9c1c`）；本轮文档与冒烟改动仍为本地工作树未提交改动，代码库处于 v0.9.5 候选阶段尚未正式发布。当前 npm 线上最新已发布稳定版本为 `v0.9.4`。
...
> - **Prometheus 可观测性体系**：补齐 LLM Token 消耗、KB 缓存命中率、MCP 工具耗时、存储池排队时间等生产度量。
> - **生产部署套件**：完善 Docker Compose 与部署配置模版。
>
