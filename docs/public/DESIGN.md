...
> 版本：v0.9.6｜设计状态：✅ 已落地 / ⚠️ 已写待补完 / 🔲 设计草案（待实现）
> 审阅视角：高级工程师 / 高级架构师
...
> **v0.9.6（2026-09-26）**：零改造本地调试：`auto_test`/`verify_ui` 对纯回环目标（本机开发服务器）默认放行（SSRF 防线不变），`auto_test` 新增宿主行动指引，README 提供宿主自定义指令片段。公开 MCP 工具、REST 契约与 schema 不变。npm 五包 `latest=0.9.6`；GitHub Release 含三平台资产。
