...
> 版本：v0.9.5（已发布稳定版）｜设计状态：✅ 已落地 / ⚠️ 已写待补完 / 🔲 设计草案（待实现）
> 审阅视角：高级工程师 / 高级架构师
...
> **v0.9.5（2026-09-26，已发布稳定版）**：虚拟/非本地堆栈帧在规范根查找前被过滤；KB SQLite 默认路径改为跨平台用户数据目录并安全迁移有效旧库；默认 HTTP 端口冲突时允许 MCP stdio-only 启动，显式端口冲突仍失败。公开 MCP 工具、REST 契约与 schema 不变。npm 五包 `latest=0.9.5`；GitHub Release 含三平台资产。
