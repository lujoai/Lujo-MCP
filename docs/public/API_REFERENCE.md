...
> 当前版本：v0.9.6（2026-09-26）。本版零改造本地调试：`auto_test`/`verify_ui` 对纯回环目标（本机开发服务器）默认放行（SSRF 防线不变），`auto_test` 新增宿主行动指引，README 提供宿主自定义指令片段。公开工具面、REST 契约、SDK 接口与数据库 schema 不变；详细变更见 [CHANGELOG](./CHANGELOG.md)。上一版 v0.9.5 为虚拟堆栈帧过滤与 KB SQLite 用户数据目录收敛。
...
- [4. Node SDK（npm 稳定版 v0.9.6）](#4-node-sdk)
- [5. 常用字段速查](#5-常用字段速查)
