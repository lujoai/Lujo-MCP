...
- [4. Node SDK（v0.7.9 已发布）](#4-node-sdkv079-已发布)
- [5. 常用字段速查](#5-常用字段速查)
...
| GET | `/api/dashboard/errors/history` | viewer | 查询错误历史（PG 长期历史，PG 不可用返回空）。注：PostgreSQL 为显式可选的**实验性**后端（不承诺支持、存在已知未修问题），默认后端为 memory，详见 POSTGRESQL_FIX_GUIDE.md |
...
## 4. Node SDK（v0.7.9 已发布）

包名固定为 `@lujoai/lujo-mcp-node-sdk`，当前发布版本为 v0.7.9，支持 Node 18、20、22，提供 CommonJS 和 ESM 根入口。该包已随本版发布，`engines.node` 为 `>=18`。
