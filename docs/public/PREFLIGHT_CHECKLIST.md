...
**适用版本 / Applicable Version**: v0.9.2
...
> **发布状态**：v0.9.2 已发布（npm registry `latest=0.9.2`）。默认 `STORAGE_BACKEND=memory`（唯一合法值，PostgreSQL 后端已正式移除）；KB 经验默认写穿本地 SQLite「笔记本」（`KB_PERSIST_ENABLED=true`，可用 `KB_PERSIST_PATH` 指定位置）。**升级须知（v0.9.2 行为变更）**：默认仅监听 `127.0.0.1`（容器须显式 `HOST=0.0.0.0`）、`/metrics` 免鉴权豁免仅回环生效、关闭期错误码 `TOOL_BUSY`、RBAC 鉴权拒绝码 `AUTH_ERROR -32003`。发布页：https://github.com/lujoai/Lujo-MCP/releases/tag/v0.9.2。
