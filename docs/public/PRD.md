...
| 项目 | 说明 |
| --- | --- |
| 文档版本 | v7.7（v0.9.5 候选同步） |
| 产品名称 | Lujo-MCP |
| 当前产品版本 | v0.9.4（已发布稳定版）/ v0.9.5（代码库候选/未发布） |
| 文档状态 | 待发布 / 候选阶段（Candidate / Pending Release） |
| 创建日期 | 2026-07-07 |
| 最后更新 | 2026-09-24 |
| 负责人 | AI 调试平台团队 |
| 审阅视角 | 高级工程师 / 高级架构师（代码核实） |

> **v0.9.5 状态（2026-09-24，代码库候选/未发布）**：优化 Trae / Cursor 智能体协同交互，消除手动调用的认知负担；虚拟帧扫描守卫防目录遍历超时核心修复已合入 main 分支（commit `390849f` / `2dc9c1c`）；本轮文档与冒烟改动仍为本地工作树未提交改动，v0.9.5 处于候选阶段尚未正式发布。当前 npm 线上最新已发布稳定版本为 `v0.9.4`。
...
| v7.7 | 2026-09-24 | 架构委员会 | **v0.9.5 候选同步（待发布）**：优化 Trae / Cursor 智能体协同交互，消除手动调用 MCP 的误解；补齐前端一次性 SDK 接入指引与安全警告；虚拟帧扫描守卫核心修复已合入 main 分支（commit `390849f` / `2dc9c1c`）；本轮文档与冒烟改动仍为本地工作树未提交改动，v0.9.5 处于候选阶段尚未正式发布。当前 npm 线上最新已发布稳定版为 v0.9.4。 |
