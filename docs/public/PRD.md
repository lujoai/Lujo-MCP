...
| 项目 | 说明 |
| --- | --- |
| 文档版本 | v7.8（v0.9.5 发布同步） |
| 产品名称 | Lujo-MCP |
| 当前产品版本 | v0.9.5（已发布稳定版） |
...
> **v0.9.5 状态（2026-09-26，已发布）**：包含三项更新：虚拟/非本地堆栈帧不再触发项目根查找与目录遍历；KB SQLite 默认文件迁至各平台用户数据目录，并在工作目录存在有效旧库时迁移一致性快照；默认 HTTP 端口冲突时保留 MCP stdio 可用。npm 五包 `latest=0.9.5`；GitHub Release 含三平台资产。详见 [CHANGELOG](./CHANGELOG.md) 与 [GitHub Release](https://github.com/lujoai/Lujo-MCP/releases/tag/v0.9.5)。
