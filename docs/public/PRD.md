...
> **v0.9.4 状态（2026-09-23，已发布）**：运行时上下文断层修复（异步修复任务补位 trace_data 异常）、MCP stacktrace 工具指定 request_id 查询真实栈帧还原、追踪摘要提取识别 trace_data、Dashboard 事件总线线程安全加固；便捷调试入口 POST /debug 废弃收敛到 POST /api/debug/run（返回 410）。发布证据：npm registry `latest=0.9.4`；GitHub Release：https://github.com/lujoai/Lujo-MCP/releases/tag/v0.9.4。
