...
> **v0.9.6 状态（2026-09-26，已发布）**：零改造本地调试：`auto_test`/`verify_ui` 对纯回环目标（本机开发服务器）默认放行（SSRF 防线不变），`auto_test` 新增宿主行动指引，README 提供宿主自定义指令片段。npm 五包 `latest=0.9.6`；GitHub Release 含三平台资产。详见 [CHANGELOG](./CHANGELOG.md) 与 [GitHub Release](https://github.com/lujoai/Lujo-MCP/releases/tag/v0.9.6)。上一版 v0.9.5：虚拟/非本地堆栈帧过滤、KB SQLite 用户数据目录迁移、默认端口冲突保留 stdio。
...
| v7.9 | 2026-09-26 | 架构委员会 | **v0.9.6 发布同步**：零改造本地调试：`auto_test`/`verify_ui` 对纯回环目标（本机开发服务器）默认放行（SSRF 防线不变），`auto_test` 新增宿主行动指引，README 提供宿主自定义指令片段。npm 五包 `latest=0.9.6`，GitHub Release 已发布并含三平台资产。 |
| v7.8 | 2026-09-26 | 架构委员会 | **v0.9.5 发布同步**：虚拟/非本地堆栈帧过滤、KB SQLite 用户数据目录与旧库快照迁移、默认 HTTP 端口冲突时保留 stdio。npm 五包 `latest=0.9.5`，GitHub Release 已发布并含三平台资产。 |
| v7.7 | 2026-09-26 | 架构委员会 | **v0.9.5 发布前候选同步**：虚拟/非本地堆栈帧过滤、KB SQLite 用户数据目录与旧库快照迁移、默认 HTTP 端口冲突时保留 stdio。 |
| v7.6 | 2026-09-23 | 架构委员会 | **v0.9.4 同步**：运行时上下文断层修复（异步修复任务补位 trace_data 异常）、MCP stacktrace 工具指定 request_id 查询真实栈帧还原、追踪摘要提取识别 trace_data、Dashboard 事件总线线程安全加固；便捷调试入口 POST /debug 废弃收敛到 POST /api/debug/run（返回 410）。测试基线 unit 2512/7、integration 105/20、e2e 24/1、Node SDK 17、SDK JS 59。产品版本 v0.9.3 → v0.9.4。 |
| v7.5 | 2026-09-23 | 架构委员会 | **v0.9.3 同步**：Qdrant 语义召回修复（qdrant-client 1.16 移除 `search()`，切换 `query_points()`，依赖下限 `>=1.10.0`）；CI 与发布产物共用锁定依赖集。无破坏性行为变更，公开工具面 18/22、Node SDK 接口、schema 不变。产品版本 v0.9.2 → v0.9.3。 |
| v7.4 | 2026-09-22 | 架构委员会 | **v0.9.2 同步**：安全加固与经验闭环——默认监听收紧为 `127.0.0.1`（容器须显式 `HOST=0.0.0.0`）、`/metrics` 免鉴权豁免仅回环生效、错误码规范化（关闭期 `TOOL_BUSY`；RBAC 鉴权拒绝 `AUTH_ERROR -32003`）、KB 存储边界拒绝未脱敏写入（ADR-20260920）、认证 fail-closed 补强、Agent 外发脱敏；新增 KB 诊断经验关联（`related_experiences`）与 `/demo` 接入状态面板；静默失败指纹隔离、stdio 错误语义补齐、Redis L2 陈旧回流阻断、双端 SDK 上报体积硬截断。v0.9.2 已发布（npm `latest=0.9.2`，GitHub Release 含三平台资产）。公开工具面 18/22、Node SDK 接口、schema 不变。产品版本 v0.9.1 → v0.9.2。 |
