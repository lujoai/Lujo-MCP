...
> **v0.9.5 状态（2026-09-26，代码库候选/未发布）**：包含三项更新：虚拟/非本地堆栈帧不再触发项目根查找与目录遍历；KB SQLite 默认文件迁至各平台用户数据目录，并在工作目录存在有效旧库时迁移一致性快照；默认 HTTP 端口冲突时保留 MCP stdio 可用。当前 npm 线上最新已发布稳定版本为 `v0.9.4`。详见 [CHANGELOG](./CHANGELOG.md)。
...
| v7.7 | 2026-09-26 | 架构委员会 | **v0.9.5 候选同步（待发布）**：虚拟/非本地堆栈帧过滤、KB SQLite 用户数据目录与旧库快照迁移、默认 HTTP 端口冲突时保留 stdio。公开 MCP 工具面、REST 契约与 schema 不变；npm 线上最新已发布稳定版为 v0.9.4。 |
