...
| v7.0 | 2026-09-09 | 架构委员会 | **v0.7.6~v0.7.8 发布同步**：(1) v0.7.6 本地调试链路稳定性与发行加固（error_id/caller_trace_id 现场可关联、session_id 契约统一、sourcemap column 保留、诊断会话校验 fail-closed、heavy 工具超时强杀、isError 语义统一、npm 默认统一模式）；(2) v0.7.7 「让宿主 AI 读对现场」——`diagnose_issue.missing_evidence` 证据缺口提示 + 7 项正确性修复 + integration 与真实 Chromium e2e 纳入 CI 门禁；(3) v0.7.8 发布工程收口（actions 升版消 Node 20 告警、平台包 `engines` 声明修复并随发布生效、生成器严格参数校验）+ 使用面文档（README 多项目「端口即隔离」示例、单用户本地自用定位声明：npm 元包装完即用，数据不出本机，不承诺中央共享数据库隔离）。注册工具 22 个 / 公开 18 个不变；测试基线 unit 1599 passed / 6 skipped、integration 75 passed / 40 skipped、e2e 9 passed / 1 skipped、SDK JS 54/54。产品版本 v0.7.5 → v0.7.8。 |
