...
- **虚拟/非本地帧不再进入规范根查找与目录遍历**：`get_related_specs` 未显式传入 `project_root` 时，先经 `_is_scannable_source_path` 过滤——拒绝空路径、`<...>` 伪帧、含 URL scheme（`://`）的页面地址，以及 UNC / 设备命名空间路径（`\\`、`//`、`\/`、`/\` 前缀，含 `\\.\` 与 `\\?\`）；本地不存在的文件同样提前返回空结果，被拒输入不产生任何文件系统访问（不触达 `os.path.isfile` / `resolve` / `os.walk`）。显式传入 `project_root` 的调用方行为不变。
...
- **发布后 C 批生命周期加固已进入 main**：执行器槽位记账、heavy 子进程结果通道、进程树终止、绝对退出死线与 lifespan 异常清理已完成，并通过 GitHub CI run 34675974282；该内容已随后续版本发布出厂（v0.9.1 起，当前最新为 v0.9.2）。
...
- **`diagnose_issue` 调试上下文新增可选字段 `missing_evidence`**：按堆栈、源码片段、运行时快照、git 归因、网络请求链、UI 事件、规范校验七个维度判定证据是否真实存在，缺哪项就给出可执行的补齐建议（调用 `stacktrace`、`get_network_trace`、`get_blame_for_frame`、`get_recent_diff`、`verify`，或检查浏览器 SDK 采集配置）。宿主 AI 可据此主动补齐证据，而不是基于残缺现场猜测。证据齐备时为 `null`，旧客户端可安全忽略；缺口计算异常时自动降级为 `null`，不影响诊断主链路。

### 🔒 修复

- `verify` / `verify_ui` 的验证结论不再被误标为工具失败（`isError=true`）：其载荷本身就是结论（`matched` / `diffs` / `silent_failure`），其中的 `error` 只是原因说明；v0.7.6 引入的全局「含非空 `error` 键即失败」契约会把结论标成失败，宿主会去反复重试而不是读取结论——包括「验证不通过」这种最有价值的结论。真正的执行失败（Playwright 崩溃、子进程超时）仍正确标记，非结论形状的裸 `error` 返回仍判失败。
- 同一错误重复上报时只关联最新一次采集现场：此前同指纹报错共用 `error_id` 且别名关联把所有历史 caller 并入，网络 / UI / 控制台证据会把多次不同操作的采集拼成一份（堆栈是最新一次，网络列表却是多次的总和），诊断结论被污染。更早几次报错的数据不丢，可用各自的 trace_id 通过 `get_network_trace` / `trace` 单独查看。
- 前端堆栈 Source Map 还原不再把「缺列号」当作「第 0 列」精确还原：此前 minified 同一生成行映射到多个源码位置时，会把 `file` / `line` / 函数名与源码片段一起错指到该行第一个符号；现在此类帧原样保留生成位置，仅当该行唯一映射时才照常还原。
- stdio 传输对齐 HTTP 的入参校验、并发门控与工具指标：不进 `tools/list` 的 SDK 上报类工具（如 `ingest_console`）此前在 stdio 上完全不校验参数，缺参/类型错误会被直接执行，现返回 `error_code: INVALID_PARAMS`（正常上报路径不受影响）；重型工具（如 `verify_ui`）此前可被无界并发调用、每次拉起一个浏览器子进程，现接入轻/重型双池上限，队列满即以 `TOOL_BUSY` 快速失败，超时/异常等各路径都会正确归还槽位；`mcp_tool_*` 指标两种传输口径一致。
- 统一模式（stdio + localhost HTTP）启动前探测端口占用：端口已被另一 Lujo 实例占用时立即以明确文案退出并给出处置（关闭旧实例、`--http-port` 换端口、只需 stdio 工具时加 `--no-http`）；此前要等服务初始化完成后才由 uvicorn 报错，宿主只见进程静默退出，且浏览器 SDK 上报会全部落入另一实例的存储，本实例查到的是空现场。
- npm 启动器 `lujo-mcp-server` 现在转发终止信号并在退出兜底中带离子进程：此前宿主停止服务后冻结二进制可能成为孤儿并继续持有统一模式的 HTTP 端口，导致 SDK 上报落入旧进程、后续启动端口冲突。已知残留：Windows 宿主用 TerminateProcess 强杀时仍可能留下孤儿（下一次启动会因端口冲突得到明确报错）。
- `diagnose_issue` 描述写明「最近一条错误」的归属口径：单机模式下所有页面/标签共用同一上报服务，缺省返回跨页面的最新一次现场，同类错误重复出现取最新一次；并补充 `session_id` 过滤用法（用户明确指向某页面/会话时传入，缺省不过滤），降低宿主 AI 取错现场的概率。
- 读取追踪日志（`diagnose_issue` / `trace` 等）时，以 JSON 对象/数组字符串透传的 `data` 统一解析为结构化对象返回，内存后端与 PostgreSQL 后端结果一致；此类字符串中的敏感字段掩码统一为 `***REDACTED***`。普通文本与解析失败的字符串保持原样。

### 验证

- CI 门禁从此前仅跑单元测试，扩展为同时执行集成测试与真实 Chromium 的 Playwright e2e 全链路，并以统一的「全 skip 假绿」守卫脚本阻断流水线（不影响 npm/Release 二进制的使用方式与对外接口）。
- npm 启动器契约测试进入 CI（参数拼装、两种安装布局解析、缺失二进制、POSIX 信号转发）；修正自 v0.7.2 起从未真正执行的 browser-sdk 漂移守卫路径（找不到比对源一律判失败），发布冒烟显式执行 postinstall 安装检查与 prepublishOnly 守卫。
- 集成/e2e 测试默认强制 memory 存储并重置存储工厂单例，不再随开发者本机 `.env` 写入真实 PostgreSQL（`LUJO_TEST_STORAGE_BACKEND=postgresql` 可显式回归真库），消除用例随运行顺序漂移的假绿风险。
...
- 无必填 ID；`store=false` 仅返回草稿预览；同 `kind+target` 重复入库自动去重（防 AI 多次调用刷屏）；RBAC 为 developer（写类工具）。
- 注册后 MCP 工具总数 22（公开清单 18）。
...
- **spec_store.delete PG 删除失败日志不可见**：PG 删除失败时内存与 trace_store 已删而 PG 行残留，进程重启后 `_do_restore` 会从 specs 表把该 spec「复活」（resurrection）；此前该失败仅 debug 级日志几乎不可见、复活无从排查；现升级为 warning 并显式说明后果与补救（PG 恢复后重试删除），优雅降级行为不变。

> 批次14 背景：用户委托外部 agent 复核「保留项清单」，其标记 9 项为「误判-真bug」；经主 agent 逐项代码验证 + 子代理独立复审双确认，仅上述 1 项属实（纯观测性改进），其余 8 项不属实——逐条代码证据反驳已留痕 CODE_REVIEW.md（含外部 agent 重复了批次10 已推翻的 KB 回灌误报、verify_loop 超时轮覆盖成果在控制流上不可能等）。

### 🔒 修复（Minor 批次13：api 1 项）

- **dashboard 缓存失效与回填竞态**：`invalidate_cache`（新 trace 持久化后调用）与并发 `_collect_all_traces` 计算之间存在竞态——后者可在失效后把旧快照重新写回 L1/L2，使新 trace 最多 30s 不可见；现引入模块级 `_generation` 代际计数，计算前快照、写回前校验，计算期间发生失效则丢弃旧快照写回（正常缓存命中/写回路径不变）。

> 批次13 收尾：第 6/7 轮 Minor「可清」项已**全部清零**（12 批次 77 项修复 + 剩余项逐条留痕移出为「设计取舍/低风险」，见 CODE_REVIEW.md）。

### 🔒 修复（Minor 批次12：runtime verifier 1 项）

- **SSRF 守卫放行浏览器内部 scheme**：`_install_ssrf_guard` 的 handler 此前对 `data:`/`blob:`/`about:` 子资源经 `inspect_url_security` 的 scheme_not_allowed 一律 abort，连带打断依赖内联图片/JS 生成内容的正常页面渲染；现对这三类浏览器内部 scheme（不发起网络请求、不构成 SSRF 面）直接放行，http/https/ws/file 等仍走逐跳 SSRF 校验（内网/回环 http(s) 拦截不受削弱）。

> 批次12 复审留痕：第 6/7 轮「需复核」4 项全部定性——trace_repo rebuild 忽略 session_id 为低风险（trace_id 为 UUID 不可猜、session 过滤仅限内存缓冲、非安全边界）；流式 fallback JSON 拼接与 kb_vector_index_autosync 语义为设计取舍（需产品确认后单独立项），均已留痕移出。

### 🔒 修复（Minor 批次11：agent + runtime storage 3 项）

- **并行 Agent 超时/异常兜底 duration_s 污染审计**：`_run_parallel_agents` 超时兜底与异常兜底中 `AgentResult.started_at` 此前用 `0.0` 哨兵，`_trace()` 的 `duration_s = finished_at - 0 ≈ 1.76e9`（Unix 纪元秒）污染 `agent_trace` 审计数组；现 `started_at` 与 `finished_at` 同取当前时间（duration_s=0）。
- **asyncpg `_ensure_init` DDL 非原子**：此前逐条 autocommit，中途失败（如 CREATE INDEX 报错）留半建 schema（表已建、索引缺）；现用 `async with conn.transaction()` 包裹，DDL 全部成功才提交、任一失败整体回滚，对齐同步 `pg_executor` 的单事务语义。
- **asyncpg `close_pool` 补重置 `_initialized`**：与 b6-5 同步侧修复同口径——池关闭后不应仍标记已初始化，进程不退出时后续 `_ensure_init` 不被短路。

### 🔒 修复（Minor 批次10：rag 1 项）

- **qdrant embedding 外发应用 `redaction_extra_patterns`**：`_redact_for_embedding` 此前只应用内联 `_REDACT_RULES`，用户自定义的 `redaction_extra_patterns`（换行分隔的额外正则）在 embedding 外发路径漏脱敏；现内联 `_load_extra_redact_rules`（与 redaction.py 同语义，架构冻结禁止 rag→runtime import 故内联），默认规则之后追加应用。

> 批次10 复审留痕：① 原计划的「启动回灌+种子后 KB 超 max_entries」经子代理复审核实为**无实际危害**——生产启动顺序为「先回灌（KB 空）后种子」，upsert 路径已兜住总上限，learned 名额 55 为种子优先的设计取舍，故撤销该修复（宁缺勿凑）。
...
- **vscode:// URL Windows 反斜杠转义**：`make_ide_link` 的 `vscode://file/{path}` 中 Windows 反斜杠在 URL 里非法（`\U`/`\A` 被当转义序列，客户端无法解析）；现统一转正斜杠（vscode 协议兼容正斜杠路径）。
- **SOURCE_PATH_MAP Windows 盘符冲突**：`pair.split(":", 1)` 在 Windows 上把盘符冒号当分隔符（`C:\app:C:\local` 被拆成 remote=`C`、local=`\app:C:\local`），映射失效；新增 `_split_remote_local` 盘符感知拆分（code_locator 与 static_analyzer 同口径复用）。
- **memory_store 单 request_id 条目无上限**：此前仅 request_id 数量有上限（`_max_entries`），单个 request_id 的 entry 列表无界（长 trace/恶意刷单条可无界涨内存）；现按「保留最新 5000 条、丢最旧」截断。
- **cleanup_expired 返回语义契约文档化**：memory 返回 request_id 数、PG 返回删除行数（同一 request_id 多行时两者不等），base.py docstring 补注——当前无消费方依赖该数值，仅作日志/诊断。
- **vector_store_min_score 边界统一**：retriever 向量召回此前用 `<=` 等值丢弃，与 kb_integration（`<` 等值保留）及 qdrant 后端（`>=` 保留）不一致，score 恰等于阈值的召回被漏掉；统一为 `<`（等值保留）。

> 批次8 复审观察（非阻塞，留痕）：① `file://` 分支（ide_scheme=file）在 Windows 仍残留反斜杠，属同类隐患、留后续；② `_split_remote_local` 对「单字母+冒号」前缀按盘符处理，但 SOURCE_PATH_MAP 的 remote 实际总是绝对路径，不会误触发。
