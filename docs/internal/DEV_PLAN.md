...
> 最近更新：2026-07-19
> 当前进度：v0.3.0 发布就绪；代码审计完成（8.0/10），Git 整理完成，准备推送
...
| 优先级 | 编号 | 说明 | 状态 |
|--------|------|------|------|
| P0 | `C3` | `trace_repo` 兜底存储键与返回 ID 不统一 | ✅ 已完成（任务 A，2026-07-19，单测 6 用例全绿） |
| P0 | `C4` | PG 持久化与 trace 回读链路缺失 | ✅ 已完成（任务 A，2026-07-19，单测 3 用例全绿；PG 集成测试待环境复核） |
...
| P1 | `N3` | stdio 生命周期资源回收 | ✅ 已完成（2026-07-19，8 用例 6 passed/2 skipped，N3 范围零回归） |
| P1 | `M1` | storage factory 拼写错误静默回退 | ✅ 已完成 |
| P1 | `M4` | JSON-RPC 错误码不规范 | 🔲 待处理 |
| P1 | `M12` | 依赖管理拆分运行时与开发时依赖 | ✅ 已完成 |

### 当前执行顺序

1. ~~先完成剩余 P0：`C3`、`C4`~~（任务 A 已完成，H10 已完成待复核，H12 已完成）
2. 再对已完成待复核项做专项验证：`H4`、`H5`、`N4`、`H10`、`H12`、`C3`、`C4`
3. 最后处理 P1：`N2`、`M4`
