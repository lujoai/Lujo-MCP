...
> **当前版本：v0.9.6（零改造本地调试）**：`auto_test` / `verify_ui` 对**纯回环目标（http://localhost:3000 等本机开发服务器）默认放行**——从本版起「配好 3 行 npx + 对宿主 AI 说哪里有问题」即可零配置调试本地前端，无需 `UI_URL_ALLOW_PRIVATE`；非回环内网/元数据地址仍默认拒绝（SSRF 防线不变）。`auto_test` 工具描述新增宿主行动指引，README 提供[可粘贴的宿主自定义指令](#让宿主-ai-学会自主使用-lujo可选复制粘贴即用)。详细说明见 [CHANGELOG](./docs/public/CHANGELOG.md)。
...
> **版本说明**：省略版本后缀时 npx 会默认拉取 npm 上的稳定最新版 `latest`（当前发布版为 0.9.6）。
>
...
```bash
npm install -g @lujoai/lujo-mcp@0.9.6
```
...
| 客户端 | 界面操作与配置文件位置 |
|---|---|
| **Trae** | **界面操作**（菜单入口与配置路径可能随 Trae 版本更新而变化，请以当前 UI 为准）：<br>点击聊天框上方的 `MCP Servers` 图标（或 `Settings` → `Features` → `MCP`）→ 点击 `Add (添加)`，填入：<br>• **Name**: `lujo`<br>• **Command**: `npx`<br>• **Args**: `-y @lujoai/lujo-mcp@0.9.6`<br>**配置文件编辑**：若支持直接编辑配置文件，常见位置为 `.trae/mcp.json`（工作区）或 `~/.trae/mcp.json`（全局），该路径随版本演进可能不同，未验证的配置路径不保证永久有效，建议以 Trae 当前设置界面或官方最新文档为准 |
...
```json
{
  "mcpServers": {
    ...
    "lujo-project-b": {
      "command": "npx",
      "args": ["-y", "@lujoai/lujo-mcp@0.9.6", "--http-port", "8102"]
    }
  }
}
```
...
**3. 只做协议冒烟、不需要浏览器现场时用 `--no-http`**：`args: ["-y", "@lujoai/lujo-mcp@0.9.6", "--no-http"]`。此时每个宿主窗口各自一个 Lujo 进程，默认 memory 后端下数据天然按进程隔离，无需端口规划。
