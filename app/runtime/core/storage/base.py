...
class TraceStorage(ABC):
    ...
    def cleanup_expired(self, ttl_seconds: int) -> int:
        ...
        FIX(v0.7.1-b8-3): 返回值的精确语义由后端决定。当前唯一运行时后端是
        memory（``MemoryTraceStore``），返回清理的 **request_id 数**；无消费方
        依赖该数值，仅作日志/诊断用途。（W13 文案更正：原文让人「去改 PG 实现」
        以统一口径，而 PostgreSQL 运行时后端已随 Step 3 移除。）
        """
        ...
