...
- `bin/lujo-mcp-server.exe` —— PyInstaller 打包的 Lujo-MCP 服务端（stdio MCP + 本地 HTTP 采集入口）。
- 本包**不声明 `bin` 字段**：可执行入口只由元包的 `bin/cli.js` 按固定路径
  `bin/lujo-mcp-server.exe` 定位。与元包同名的 bin 会让 npm 在同一安装树跳过全部
  同名链接，导致 `node_modules/.bin` 为空。

其他平台包：`@lujoai/lujo-mcp-linux-x64`、`@lujoai/lujo-mcp-osx-arm64`。
