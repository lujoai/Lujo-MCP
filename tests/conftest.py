...
历史 .env 曾含 `POSTGRES_PASSWORD`、`DATABASE_URL` 等供 docker-compose 使用的字段
（PG 配置族已在 Step 3 WP6 删除；遗留键现经 `app/config.py` 的 `extra="ignore"`
被忽略），这些字段未在 `app/config.py::Settings` 中声明，导致 `Settings()` 实例化
即崩，所有测试无法收集。这是 v0.3.0 Release Audit 清单中 M9（`.env` 出现未知键
启动即崩）在测试环境下的表现，本身是待处理项（P2），不在本任务修复范围。
